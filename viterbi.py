...
def most_likely_tag(train_set: list, test_set: list) -> (float, float, float):
    ...
    known_words_error_rate = known_words_prediction_wrong / (
        known_words_prediction_wrong + known_words_prediction_right)
    unknown_words_error_rate = unknown_words_prediction_wrong / (
        unknown_words_prediction_wrong + unknown_words_prediction_right)
...
def viterbi_algorithm(sentence: list, transition_matrix: dict, emission_matrix: dict) -> list:
    """
    ...
    if did_all_the_word_appeared:
        ...
        all_tags = S[1]
        return tags_list + random.sample(all_tags, len(sentence) - last_good_word_index - 1)
...
a = viterbi_algorithm(["The", "jury", "said", "it", "did"], transmission, emission)
print(a)
